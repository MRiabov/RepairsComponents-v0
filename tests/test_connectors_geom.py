...
# dev note: the logic is that terminal def is at the same position as connector.
...
def test_terminal_pos_in_connected_connectors_match(all_connectors: list[Connector]):
    for connector_dataclass in all_connectors:
        bd_geometry_male, terminal_pos, bd_geometry_female, _ = (
        ...
        ##hmmm. this isn't expected to work either because the terminal def is at

        no_terminal_def_male = Compound(children=[bd_geometry_male.children[0]])
        no_terminal_def_female = Compound(children=[bd_geometry_female.children[0]])
        cleaned_up_intersection_compound = Compound(
            children=[no_terminal_def_male, no_terminal_def_female]
        )
...
def test_connector_pos_relative_to_center_match(all_connectors: list[Connector]):
    for connector_dataclass in all_connectors:
        ...
        assert np.isclose(
            tuple(bd_geometry_male.children[-1].center(CenterOf.BOUNDING_BOX)),
            connector_dataclass.terminal_pos_relative_to_center_male,
        ).all(), (
            f"Male terminal pos relative to center doesn't match. It is set as {connector_dataclass.terminal_pos_relative_to_center_male}"
            f" but it is {bd_geometry_male.children[-1].center(CenterOf.BOUNDING_BOX)}"
        )
        assert np.isclose(
            tuple(bd_geometry_female.children[-1].center(CenterOf.BOUNDING_BOX)),
            connector_dataclass.terminal_pos_relative_to_center_female,
        ).all(), (
            f"Female terminal pos relative to center doesn't match. It is set as {connector_dataclass.terminal_pos_relative_to_center_female}"
            f" but it is {bd_geometry_female.children[-1].center(CenterOf.BOUNDING_BOX)}"
        )
        # ^ now, this is expected to work
        assert (
            connector_dataclass.terminal_pos_relative_to_center_male[1:]
            == np.array([0, 0])
        ).all(), "expected y and z of connector defs to be 0"
        assert (
            connector_dataclass.terminal_pos_relative_to_center_female[1:]
            == np.array([0, 0])
        ).all(), "expected y and z of connector defs to be 0"
...
def test_naming_and_colors(all_connectors: list[Connector]):
    for connector_dataclass in all_connectors:
        ...
        # terminal def names
        assert bd_geometry_male.children[-1].label.endswith("@terminal_def")
        assert bd_geometry_female.children[-1].label.endswith("@terminal_def")
        # connector names
